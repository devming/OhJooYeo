...
extension DbManager {
    ...
    func addWorshipOrder(worshipOrders: [Model.WorshipOrder]) {
        if let newPhrase = NSEntityDescription.insertNewObject(forEntityName: DbManager.Name.worshipOrderEntityName, into: defaultContext) as? WorshipOrderEntity {      // type casting을 해서 내가 사용할 엔티티를 가져와야한다.
            for worshipOrder in worshipOrders {
                ...
                newPhrase.order = "\(worshipOrder.order)"
                saveContext()
            }
        }
    }
    ...
    func getWorshipOrderList() -> [WorshipOrderEntity] {
        // 1. NSFetchRequest
        let request = NSFetchRequest<WorshipOrderEntity>(entityName: DbManager.Name.worshipOrderEntityName)
