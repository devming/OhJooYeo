...
extension DbManager {
    func addMusic(musics: [Model.Music]?, worshipMO: WorshipMO?) {
    ...
    func updateMusic(musics: [Model.Music]?, worshipMO: WorshipMO?) {
        guard let musics = musics, let worshipMO = worshipMO else {
            return
        }
    ...
    private func saveMusicToContext(_ musics: [Model.Music], worshipMO: WorshipMO) {
        for music in musics {
